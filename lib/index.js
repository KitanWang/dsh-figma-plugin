 ...
 * bundled design-to-code skills, and one browser page under Settings that signs
 * in to Figma with OAuth. Sign-in uses an OAuth app built into the plugin
 * (lib/oauth-app.js), so the user never registers an application and never
 * handles a token.
 *
 * The tools never reach into the harness's own filesystem or network seams:
 * they own their HTTP client, their grant handling, and their export files,
 * and they degrade cleanly when an optional service (attachments, skills,
 * credentials, web server) is not mounted.
 *
 * @module dsh-figma
 */
...
/**
 * Plugin config. Every key is optional: with no config at all the plugin signs
 * in with its built-in OAuth app, refreshes its own grant, and writes exports
 * under `<workspace>/.dsh-figma`.
 */
export const Config = z.object({
  ...
  // ── OAuth connection ──────────────────────────────────────────────────────
  /**
   * Client ID of the plugin's OAuth app. Deployments normally leave this empty
   * and rely on `lib/oauth-app.js`; it exists so a fork can point at its own app.
   */
  clientId: z.string().default(''),
  /** Client Secret of the plugin's OAuth app; empty falls back to `lib/oauth-app.js`, then the environment. */
...
async function resolveToken(connection, options = {}) {
  try {
    return await connection.accessToken(options);
  } catch (error) {
    // The tool layer reports Figma failures as FigmaApiError, and the
    // connection already writes a user-facing message.
...
async function clientFor(connection, config, options = {}) {
  const { token } = await resolveToken(connection, options);
...
function wireConnectionRoutes(ctx, connection, config) {
  ...
  // The redirect URL must name the port this server actually bound. The route
  // below lives on this same server, so any other value would send Figma to a
  // port where nothing is listening. A deployment behind a reverse proxy sets
  // `redirectUri` to its public URL instead.
  const redirectUri = isLoopbackRedirect(config.redirectUri)
    ? config.redirectUri
    : `http://${webServer.host === '0.0.0.0' ? '127.0.0.1' : webServer.host}:${String(webServer.port)}${callbackPath}`;
  ...
  ctx.effect(
    () =>
      webServer.register({
        kind: 'prefix',
        path: API_BASE,
        handler: createApiHandler(connection, { prefix: API_BASE }),
      }),
...
export function apply(ctx, config) {
  ...
  // Injection, not a direct `ctx.get`: the web server is a peer row, so it may
  // mount after this plugin and the contributions must appear when it does. A
  // context without `inject` (a minimal embedding) reads whatever is already
  // mounted instead of waiting.
  ...
  registerTool(ctx, resolved, tools.whoami !== false, {
    name: 'figma_whoami',
    description: 'Verify the Figma connection and return the authenticated account.',
    parameters: {},
    output: {
      schema: {
        type: 'object',
        additionalProperties: false,
        properties: {
          verified: { type: 'boolean', required: true, description: 'Whether the connection authenticated.' },
          id: { type: 'string', description: 'Figma account id; absent when the grant cannot read the account.' },
          handle: { type: 'string', description: 'Figma handle; absent when the grant cannot read the account.' },
          email: { type: 'string' },
          note: { type: 'string', description: 'Present when the grant authenticated but is narrowly scoped.' },
        },
      },
      render: (_args, value) =>
        value.handle === undefined
          ? [
              {
                type: 'text',
                text: `The Figma connection authenticates, but the account could not be read.${value.note === undefined ? '' : ` ${value.note}`}`,
              },
            ]
          : [
              {
                type: 'text',
                text: `Authenticated with Figma as ${value.handle}${value.email === undefined ? '' : ` <${value.email}>`}.`,
              },
            ],
    },
    timeoutMs,
    ...
    async execute(_args, exec) {
      const { token } = await resolveToken(connection, { signal: exec.signal });
      ...
      if (error.status === 401) {
        throw new Error('the Figma grant is invalid or has been revoked; reconnect from Settings → Figma');
      }
      if (error.status === 403) {
        ...
        return {
          verified: true,
          note: 'the grant lacks the current_user:read scope, so the account cannot be read; file, style, component, and comment access is unaffected',
        };
      }
      throw error;
    },
      ...
      render: (_args, value) => [
        {
          type: 'text',
          text: [
            value.connected ? 'Figma is connected.' : 'Figma is not connected.',
        ...
        return {
          ...
          nextStep: status.connected
            ...
            : status.available
              ...
              : 'This build carries no Figma OAuth client, so sign-in is unavailable; a deployment must supply one.',
        };
      }
      if (status.connected) {
        return {
