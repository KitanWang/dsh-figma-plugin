window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			setupTitleOptional: "可选：改用 OAuth 登录（令牌自动续期）",
			setupDescOptional: "当前的个人访问令牌已经可用。OAuth 登录的好处是 access token 会自动续期，且不依赖长期有效的个人令牌。需要的话按下面三步配置。",
		...
		const en = {
			...
			setupTitleOptional: "Optional: switch to OAuth sign-in (tokens refresh automatically)",
			setupDescOptional: "Your personal access token already works. Signing in with OAuth additionally refreshes the access token automatically and avoids depending on a long-lived personal token. Set it up with the three steps below if you want that.",
		...
		function createConnectionStore() {
			...
			return {
				...
				/**
				 * Persist credentials and, when asked, continue straight into
				 * authorization.
				 *
				 * Saving and connecting are one server round trip because that route
				 * commits the credentials before starting the attempt. Returning the
				 * URL lets the caller open Figma's page — otherwise the attempt would
				 * sit pending with nobody sent to consent.
				 */
				async saveClient(clientId, clientSecret) {
					...
					if (result.error !== undefined) {
						...
						return { saved: false, authorizationUrl: null };
					}
					await refresh({});
					return { saved: true, authorizationUrl: result.authorizationUrl ?? null };
				},
		...
		function SetupForm(props) {
			...
			// When a personal access token already works, this is an upgrade the
			// user may skip, not a blocker.
			...
			return React.createElement(
				...
				React.createElement("h3", { className: "figma-setup-title" }, t(optional ? "setupTitleOptional" : "setupTitle")),
				React.createElement("p", { className: "figma-muted" }, t(optional ? "setupDescOptional" : "setupDesc")),
				React.createElement(
					"ol",
					...
					React.createElement(
						...
						React.createElement(
							Button,
							{
								...
								onClick: async () => {
									const result = await store.saveClient(clientId, clientSecret);
									if (!result.saved) return;
									setClientSecret("");
									setSaved(true);
									setTimeout(() => setSaved(false), 1500);
									// Saving already starts the attempt server-side, so the
									// page must open now or the attempt waits for nobody.
									if (result.authorizationUrl !== null) {
										let opened = null;
										try {
											opened = window.open(result.authorizationUrl, "_blank", "noopener,noreferrer");
										} catch {
											opened = null;
										}
										if (opened === null) store.notifyBlocked(result.authorizationUrl);
										store.startPolling();
									}
								},
							},
							saved ? t("saved") : t("saveAndConnect"),
						),
					),
				),
			);
		...
		function ConnectionPanel(props) {
			...
			// The client setup is required only when nothing else can authenticate.
			// With a working personal access token it is an optional upgrade, so
			// the copy must not read as a blocking prerequisite.
			const needsSetup = status.oauthSupported === true && status.clientConfigured !== true;
			if (needsSetup) {
				children.push(React.createElement(SetupForm, { t, store, status, optional: connected, key: "setup" }));
			}
