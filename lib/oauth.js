 ...
 *
 * Figma rejects an authorization request outright when it asks for any scope
 * the OAuth app does not have enabled — it answers "Invalid scopes for app" and
 * the browser shows that instead of a consent page. This list must therefore
 * stay a subset of what the app's OAuth scopes page configures.
 *
 * It is deliberately the smallest set that covers the tools:
 *
 * - `current_user:read` — `figma_whoami` and the connected account
 * - `file_content:read`, `file_metadata:read` — files, nodes, styles, components
 * - `file_comments:read`, `file_comments:write` — `figma_get_comments`, `figma_post_comment`
 * - `file_dev_resources:read` — `figma_get_dev_resources`
 * - `library_content:read`, `library_assets:read` — published library entries
 *
 * `file_variables:read` is deliberately absent. Figma marks it Enterprise-only,
 * and a plan that does not offer a scope cannot enable it on the app either —
 * so requesting it would fail the whole authorization on every other plan, and
 * losing every tool to enable one is a bad trade. An Enterprise deployment that
 * wants `figma_get_variables` adds it through the `scopes` plugin config, after
 * enabling it on the app.
 */
...
/** The Enterprise-only scope a deployment may opt into through the `scopes` config. */
export const ENTERPRISE_VARIABLES_SCOPE = 'file_variables:read';
