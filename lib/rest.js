 ...
 *
 * One client instance is bound to one bearer token; callers resolve the token
 * per tool call, so a grant refreshed in the harness store takes effect on the
...
export class FigmaClient {
  ...
  /**
   * @param options - bearer token, base URL, timeout, retry budget.
   */
  constructor(options = {}) {
   ...
   *
   * An OAuth grant is always a bearer credential — there is no second auth
   * mode, because the plugin has no personal-access-token path.
   */
  #authHeaders() {
