...
/** Cap on a JSON request body from the panel. */
 ...
 * @returns the decoded object, or undefined when the body is empty.
 * @throws when the body exceeds the cap or is not JSON.
 */
  ...
  return async function handleApi(request, response) {
    ...
    try {
      if (route === '/status' && method === 'GET') {
        ...
        const status = await connection.status({ verify });
        // The GUI port is only knowable from the request that reached it.
        const host = request.headers.host;
        if (typeof host === 'string' && host.length > 0 && options.guiUrl === undefined) {
          status.guiUrl = `http://${host}/`;
        }
      ...
      if (route === '/connect' && method === 'POST') {
        ...
        if (body !== undefined) await connection.saveClient(body);
      ...
      if (route === '/disconnect' && method === 'POST') {
        if (!sameOrigin(request)) {
          sendJson(response, 403, { error: 'the disconnect request did not come from this GUI' });
          return;
        }
