...
The plugin needs a Figma personal access token. Create one at
**Figma → Settings → Security → Personal access tokens**
(<https://www.figma.com/developers/api#access-tokens>) with at least the
...
> Ask the agent to run `figma_whoami`.
...
| `figma_whoami` | Verify the token and report the authenticated account. |
...
| Key | Default | Meaning |
| --- | --- | --- |
| `accessToken` | `''` | Explicit token. When empty, the credential store and environment are consulted. |
| `authMode` | `token` | `token` sends `X-Figma-Token`; `oauth` sends `Authorization: Bearer`. |
...
| `tools` | all on | Per-tool switches: `whoami`, `file`, `designContext`, `screenshot`, `variables`, `styles`, `components`, `devResources`, `comments`, `postComment`. |
...
only a token and no running Figma desktop:

| | Codex + Figma plugin | dsh-figma |
| --- | --- | --- |
| Design reads | Figma MCP server (OAuth) | Figma REST API (personal access token) |
...
| Setup | install plugin, authorize Figma | install plugin, set a token |
...
```sh
npm test                    # 52 unit + integration tests, no network
node scripts/smoke.mjs      # mount in a real Cordis context and assert registration
```
...
is exercised without a Figma account. `scripts/smoke.mjs` mounts the plugin
next to the harness's real `ToolRuntime`, `SkillRegistry`, and `SystemPrompt`
services and asserts the tools, skills, and prompt section land.
